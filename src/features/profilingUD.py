...
""" Tramite questo script ho creato i file
di input da passare al tool on line per ottenere 
le profiling ud table e i file per rimappare le label,
le due porzioni di codice separate sono state commenatte alternativamente per eseguire
i due compiti:
1-> produrre input per il tool
2-> rimappare gli output del tool con le relative label 


DISCLAIMER: ai file di output del tool sono state apportate delle modifiche al nome 
delle colonne, all'id dei testi e al formato del csv"""



#PARTE 1 -> TRASFORMAZIONR FILE PER ESSERE PROCESSATI DA PROFILING UD ONLINE TOOL =========
